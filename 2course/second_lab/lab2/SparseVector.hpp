...
namespace myvector {
    ...
    void SparseVector<T>::prepend(T value) {
        this->resetUsedIndex();
        myarray::ArraySequence<Pair<int, T>> arr;
        for (unsigned int i = 0; i < this->size; ++i) {
            if (this->data->contains(i)){
         ...
         for (size_t i = 0; i<arr.length();i++) {
                this->data->insert(arr[i]->getValue().getFirst(), arr[i]->getValue().getSecond());
            }
        if (value != this->defaultValue){
    ...
    template<typename T>
    typename myarray::Array<T>::ArrayIterator SparseVector<T>::begin() {
        this->resetUsedIndex();
        this->refreshItems();
    ...
    void SparseVector<T>::refreshItems() {
        ...
        if (this->baseTypeOfSequence == "ArraySequence") {
            seq = new myarray::ArraySequence<T>();
        } else {
    ...
    template<typename T>
    typename myarray::Array<T>::ArrayIterator SparseVector<T>::end() {
